...
#
# Vendored from https://github.com/madebyollin/taehv (MIT-licensed) — the
# single TAEHV copy in this repo, used both as an opt-in TAEW decoder in the
# Re-DMD reward path and as the fast decoder in the inference scripts. Kept
# identical to the upstream taehv.py below the SPDX header, except for the
# added ``trim_output`` flag (the inference decoder wrappers handle trimming
# themselves to match Wan's frame convention).
"""
...
class TAEHV(nn.Module):
    def __init__(self, checkpoint_path="taehv.pth", encoder_time_downscale=(True, True, False), decoder_time_upscale=(False, True, True), decoder_space_upscale=(True, True, True), patch_size=1, latent_channels=16, trim_output=True):
        ...
        Arg:
            ...
            trim_output: if False, decode_video returns the raw (untrimmed) frame
              sequence; callers that align frames to Wan's convention themselves
              (the inference decoder wrappers) set this to False. [local addition]
        """
        super().__init__()
    ...
    def decode_video(self, x, parallel=True, show_progress_bar=True):
        ...
        if skip_trim or not self.trim_output:
            ...
            # (trim_output=False: caller handles trimming — local addition)
            return x
